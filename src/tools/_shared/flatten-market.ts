...
export const FLATTEN_MARKET_OPTIONAL_FIELDS = [
  ...
  'timeToMaturity',
  'timeToMaturityLabel',
  'settlementsToMaturity',
  'assetMarkPrice',
  'rateSensitivity',
  'dailyVolatility',
  'dailyVolatilityState',
  'softOICap',
  'imData',
  'config',
  'metadata',
...
export function flattenMarket(m: unknown): FlattenedMarket {
  ...
  return {
    ...
    name,
    // metadata.name is the SHORT curated pair label ("BTCUSDT") — distinct from `name`/`symbol`,
    // which come from imData ("Binance BTCUSDT 26 Sep 2025" / "BINANCE-BTCUSDT-26SEP2025").
    shortName: metadata.name,
    symbol,
    platform: (raw.platform as Record<string, unknown> | undefined)?.name,
    maturity,
    ...
    maxPerpLeverage: metadata.maxPerpLeverage,
    ammId: resolvedAmmId,
    ...
    settlementsToMaturity: data.settlementsToMaturity,
    assetMarkPrice: data.assetMarkPrice,
    rateSensitivity: data.rateSensitivity,
    dailyVolatility: data.dailyVolatility,
    dailyVolatilityState: data.dailyVolatilityState,
