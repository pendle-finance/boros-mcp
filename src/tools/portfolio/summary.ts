...
export function registerPortfolioSummaryTools(server: McpServer): void {
  server.registerTool(
    'get_portfolio_summary',
    {
      ...
      description: 'Aggregate portfolio snapshot for an account: per-collateral-token totals (collateral, equity, initial/available margin), open-position count, open-order count, margin utilization, and optionally unrealized PnL. COLLATERAL FIGURES ARE ACCOUNT ID 0 ONLY: the discovery call GET /v1/accounts/market-acc-infos-by-root hardcodes accountId 0 server-side and cannot be pointed at a sub-account, so for accountId 0 it does find every funded marketAcc (cross + isolated, including dust accounts with cash but no positions) but for ANY non-zero accountId every collateral/margin number here is 0 and `accounts` is empty — while openPositionCount / openOrderCount DO honour the accountId. A non-zero accountId showing 0 collateral next to open positions is that gap, not an empty account: use get_collateral with an explicit `marketAccs` list instead. Single read-only call that replaces fan-out over get_collateral + get_positions + get_pnl_by_market + get_orders. Use summaryByToken when summary.isCrossCollateral is true (positions in >1 token, e.g. WETH and USDT). Does NOT include the gas wallet (use get_gas_info).',
      inputSchema: {
    ...
    async ({ userAddress, accountId, includePnl }) => {
      try {
        ...
        return jsonResult({
          ...
          _context: {
            accountId: 'Collateral discovery (market-acc-infos-by-root) is accountId 0 ONLY — hardcoded server-side. For any non-zero accountId, totalCollateral / netBalance / initialMargin / availableMargin / marginUtilizationPct / summaryByToken / accounts are all EMPTY-OR-ZERO by construction and must not be read as "no funds"; openPositionCount and openOrderCount are correct for the requested accountId. For non-zero accountId collateral, call get_collateral with an explicit `marketAccs` list (source the handles from get_positions include:["marketAcc"]).',
