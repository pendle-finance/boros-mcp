...
// Canonical Boros documentation sources, served on demand by `boros_glossary` (`docs` field) so the
// model knows where to WebFetch for deeper explanations the tools don't inline. If a
// docs.pendle.finance URL 404s, drop a leading numeric prefix from the Backend/ slug and retry, or
// fall back to the boros-dev index.
export const BOROS_DOCS = {
  introduction: 'https://docs.pendle.finance/boros-dev/Introduction',
  litePaper: 'https://docs.pendle.finance/boros-dev/LitePaper',
  ...
  historicalData: 'https://docs.pendle.finance/boros-dev/Backend/historical-data',
  glossary: 'https://docs.pendle.finance/boros-dev/Backend/glossary',
  historicalDataArchive:
    'https://historical-data.boros.finance — bulk downloadable history (OHLCV, trades, order book, settlement, underlying funding) as ZIP-compressed NDJSON, refreshed every 2-3 days. GET https://historical-data.boros.finance/files.json for the {path,size} manifest; per-market folders are named {marketId}-{symbol}. Use for long-range/offline analysis beyond what get_market_ohlcv/get_market_trades/get_orderbook return per call.',
};
